...
`gsa` always produces a stable bijection between two sets of the same size. Moreover, the assignment is always best for the proposer set.

*Proof By Contradiction*: Assume there exist some `gsa` output which is unstable `=>` (by defn of stability) there exists two member of the opposite set that prefer each other to their current partner. We can assign the two pairs `[A,B],[C,D]` and, wlog, `A` prefers `D` to `B` and `D` prefers `A` to `C`. If the proposing set is `[A,C]`, then `A` would propose to `D` before `B`, and, if `C` proposes to `D` while `D` is with `A`, `D` would reject because `D` prefers `A` to `C`. Therefore, it is impossible to get `[A,B],[C,D]` from `gsa` if `[A,C]` are the proposers. Similarly, if `[B,D]` are the proposers, then `B` proposes to `A` and `A` accepts, but `D` proposes next to `A` and `A` would displace `B` to accept `D` so `[A,B],[C,D]` is an impossible outcome of `gsa` with `[B,D]` as proposers. Therefore, no unstable pair can exist in the `gsa` output because we have fully explored the `gsa` solution space by alternating the proposer set.

If all elements of the proposer set share the same preferences, the `gsa` output will pair the `i`th proposer with their `i`th preference. So, order of proposers does effect `gsa` output.
